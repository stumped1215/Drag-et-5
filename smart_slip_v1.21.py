...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.21 • Improved Grok Workflow")
...
# PREDICT + GROK (Improved Grok workflow)
with tab_predict:
    ...
    st.divider()
    st.subheader("Ask Grok (Improved)")

    # Number of recent runs to include
    num_runs = st.slider("Include how many recent runs?", 3, 10, 5)

    if st.button("Generate Prompt for Grok", type="primary", use_container_width=True):
        if vehicle_runs:
            recent = vehicle_runs[-num_runs:]
            ...
            for r in recent:
                ...
                if r.get("lift_before_330"): conditions.append(f"lift before 330'")
                if r.get("lift_after_330"): conditions.append(f"lift after 330'")
            ...
            st.code(prompt, language="text")
            st.caption("Copy the prompt above and send it to me in this chat. Paste my response below.")
...
st.divider()
st.caption("Smart Slip v1.21 • Built for bracket racers")