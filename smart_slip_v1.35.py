...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.35 • Car Profiles + Notes Focused")
...
with tab_import:
    ...
    st.info("**Important:** Put details about spinning, lifting, or braking (when and how bad) in the **Notes** section. This is very important for accurate predictions.")
...
- Put any details about spinning, lifting, or braking (when + how much) in the notes field.
...
# PREDICT + GROK (with Car Profile selection)
with tab_predict:
    st.subheader("Predict ET (Per Vehicle + Profile)")
    ...
    st.markdown("**Car Profile (for Grok context)**")
    if car_profiles:
    ...
    st.divider()
    st.subheader("Ask Grok (Full Context + Car Profile)")
    ...
    if st.button("Generate Prompt for Grok", use_container_width=True):
        if vehicle_runs:
            ...
            # Add Car Profile info if selected
            if selected_profile:
                prompt += "Car Profile Information:\n"
                prompt += f"- Name: {selected_profile['name']}\n"
                prompt += f"- Type: {selected_profile['car_type']}\n"
                prompt += f"- Fuel: {selected_profile['fuel_type']}\n"
                prompt += f"- Weight: {selected_profile['weight']} lbs\n"
                prompt += f"- Tire Size: {selected_profile['tire_size']}\n"
                prompt += f"- Tire Type: {selected_profile['tire_type']}\n"
                prompt += f"- Trans 1st Gear: {selected_profile['trans_first_gear']}\n"
                prompt += f"- Rear Gear: {selected_profile['rear_gear']}\n\n"
            
            prompt += "Recent runs (pay close attention to notes about spinning, lifting, braking):\n"
            for r in recent:
                ...
                if r.get("actual_et"): prompt += f" | Actual ran: {r['actual_et']}"
                if r.get("notes"): prompt += f" | Notes: {r['notes']}"
                prompt += "\n"
            prompt += f"\nTarget Density Altitude: {target_da} ft\n"
            prompt += "\nPlease give a smart ET prediction with reasoning. Consider the car profile and all notes."
            st.code(prompt, language="text")
...
st.divider()
st.caption("Smart Slip v1.35 • Built for bracket racers")