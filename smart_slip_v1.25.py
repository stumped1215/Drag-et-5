...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.25 • Improved Import Prompt")
...
# IMPORT FROM GROK (Improved prompt)
with tab_import:
    ...
    # Improved, stronger prompt
    prompt = f"""You are extracting data from a drag racing timeslip photo for import into Smart Slip.

Instructions:
1. Extract data ONLY from the correct side of the slip for car number {car_number or '____'}.
2. If the photo shows two lanes, clearly use only the side that matches the car number.
3. Pull historical weather for the exact date and time shown on the slip (use airdensityonline.com or aviation sources if needed).

Output format — Return ONLY key=value lines, one per line. Use these exact keys:

date=YYYY-MM-DD
time=HH:MM
track=Track Name
vehicle=Car Number + Class (e.g. Car 1258 Truck)
et=5.517
sixty_ft=1.240
eighth_et=5.517
trap_mph=125.49
...
notes=Any notes from the slip (round number, Compulink, etc.)

If a value is not visible or unclear, use "None" for that key.
Do not add any extra text or explanation — only output the key=value lines."""
...
st.divider()
st.caption("Smart Slip v1.25 • Built for bracket racers")