...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.17 • Mobile Optimized")

runs = st.session_state.runs

# Mobile-friendly sidebar (collapsed by default)
with st.sidebar:
    ...
    if st.button("Export to CSV"):
        if runs:
            ...
            st.warning("No runs to export.")

# Main content - more mobile friendly
tab_dashboard, tab_import, tab_manual, tab_predict, tab_history, tab_settings = st.tabs([
    "🏠 Home", "📥 Import", "✍️ Log", "🔮 Predict", "📜 History", "⚙️ Settings"
])
...
with tab_dashboard:
    ...
    if model:
        st.success(f"Smart model active ({model['runs_used']} runs)")
    else:
        st.info("Log 5+ runs with weather data to activate predictions.")

    st.divider()
    
    # Prominent Quick Log button
    if st.button("⚡ Quick Log (Fast Entry)", use_container_width=True):
...
if st.session_state.get("show_quick_log", False):
    ...
    with st.form("quick_log"):
        ...
        q_notes = st.text_area("Notes", height=60)
...
with tab_import:
    ...
    st.markdown("**Steps:**")
    st.markdown("1. Take photo of timeslip")
    st.markdown("2. Send to Grok with the prompt below")
    st.markdown("3. Paste the block here")

    prompt = f"""The car number is {car_number or '____'}. Extract data from the correct side and format as key=value lines for Smart Slip.
...
Fields: date, time, track, vehicle, et, sixty_ft, eighth_et, trap_mph, reaction_time, temp_f, altimeter_inhg, humidity_pct, density_altitude, notes"""
    ...
    if not st.session_state.import_success:
        block = st.text_area("Paste block from Grok", height=160, key="import_block")
        ...
        if st.button("Import Run", type="primary", use_container_width=True):
            if not block or not block.strip():
                ...
                # Parsing logic (simplified for mobile)
                data = {}
...
with tab_predict:
    ...
    if st.button("Generate Prompt for Grok", use_container_width=True):
        if vehicle_runs:
            ...
            prompt += f"\nTarget DA: {target_da} ft\nPlease give a smart prediction and safe dial-in."
            st.code(prompt, language="text")
...
with tab_history:
    ...
    if not runs:
        ...
        all_vehicles = ["All"] + sorted(list(set(r.get("vehicle", "Unknown") for r in runs)))
        selected = st.selectbox("Vehicle", all_vehicles)
        
        filtered = runs if selected == "All" else [r for r in runs if r.get("vehicle") == selected]
        
        for r in filtered[-10:][::-1]:  # Show last 10
...
st.divider()
st.caption("Smart Slip v1.17 • Built for bracket racers")