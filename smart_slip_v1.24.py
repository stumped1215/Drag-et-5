...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.24 • High-Value Updates")
...
with tab_manual:
    ...
    # New: Actual ET field for tracking
...
# PREDICT + GROK (with improved What If)
with tab_predict:
    ...
    # Improved What If section
    st.markdown("**What If Scenarios** (select multiple)")
    col_wif1, col_wif2 = st.columns(2)
    with col_wif1:
        what_if_nitrous = st.checkbox("Add nitrous this pass")
        what_if_clean = st.checkbox("Assume clean pass")
    with col_wif2:
        what_if_da_change = st.number_input("Adjust target DA by", value=0, step=50)
    ...
    if st.button("Calculate Prediction", type="primary", use_container_width=True):
        ...
        if not model or len(vehicle_runs) < 3:
            ...
            if baseline.get("density_altitude"):
                da_diff = (target_da + what_if_da_change) - baseline["density_altitude"]
                base_predicted = baseline["et"] + (da_diff / 1000) * model["sensitivity"]
                
                adjustment = 0.0
                if what_if_nitrous:
                    adjustment -= 0.08
                if what_if_clean:
                    if baseline.get("spin"): adjustment -= 0.06
                    if baseline.get("lift_before_330") or baseline.get("lift_after_330"): adjustment -= 0.05
                    if baseline.get("brakes_after_330"): adjustment -= 0.04

                final_pred = base_predicted + adjustment
                safe = final_pred + 0.02

                st.success(f"**Predicted ET: {final_pred:.3f}s**")
                st.info(f"**Recommended safe dial-in: {safe:.3f}s**")

                if what_if_nitrous or what_if_clean or what_if_da_change != 0:
                    st.caption("What If adjustments applied.")
    ...
    if st.button("Generate Prompt for Grok", use_container_width=True):
        if vehicle_runs:
            ...
            if what_if_nitrous:
                prompt += "Scenario: Adding nitrous this pass.\n"
            if what_if_clean:
                prompt += "Scenario: Assuming this is a clean pass.\n"
            if what_if_da_change != 0:
                prompt += f"Scenario: DA adjusted by {what_if_da_change} ft.\n"
...
# HISTORY (with Actual vs Predicted)
with tab_history:
...
st.divider()
st.caption("Smart Slip v1.24 • Built for bracket racers")