...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.32 • Notes-Focused (Removed Structured Special Conditions)")
...
with tab_manual:
    ...
    st.info("**Important:** Use the Notes section below to record spinning, lifting, or braking (when it happened and how bad). This is very important for good predictions.")
...
with tab_predict:
    ...
    if st.button("Calculate Prediction", type="primary", use_container_width=True):
        ...
        if not model or len(vehicle_runs) < 3:
            ...
            st.caption(f"Using {model['runs_used']} runs (R² = {model['r2']})")

    st.divider()
    st.subheader("Ask Grok (Notes + All Data)")
    ...
    if st.button("Generate Prompt for Grok", use_container_width=True):
        if vehicle_runs:
            ...
            prompt += "\nPlease give a smart ET prediction with reasoning and a recommended safe dial-in. Pay special attention to any notes about spinning, lifting, or braking."
            st.code(prompt, language="text")
...
with tab_settings:
    ...
    st.caption("v1.32 • Notes-Focused")

st.divider()
st.caption("Smart Slip v1.32 • Built for bracket racers")