...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("Bracket Racing • Formula + Grok Hybrid  •  v1.4")
...
tab_dashboard, tab_import, tab_manual, tab_predict, tab_history, tab_settings = st.tabs([
    "Dashboard", "Import from Photo", "Manual Log", "Predict + Grok", "History", "Settings"
])
...
# IMPORT FROM PHOTO
with tab_import:
    st.subheader("Import from Photo")
    st.markdown("Upload your timeslip photo to me → I extract data + weather + notes → paste the block below.")

    if not st.session_state.import_success:
        block = st.text_area("Paste the import block from Grok here:", height=200, key="import_block")
...
with tab_predict:
    ...
    st.divider()
    st.subheader("Consult Grok (Smarter Prediction)")
    if vehicle_runs:
        st.markdown(f"**Using runs from: {selected_vehicle}**")
        recent = vehicle_runs[-8:]
        selected = []
        for i, r in enumerate(recent):
            label = f"{r['date']} - ET {r['et']:.3f}s @ {r.get('density_altitude','N/A')}ft"
            if st.checkbox(label, value=True, key=f"grok_run_{i}"):
                selected.append(i)
        context = st.multiselect("Quick Context", ["Nitrous this pass", "New tune/setup", "Track prep different", "New tires"])
        extra = st.text_area("Additional notes for Grok")
        if st.button("Generate Prompt for Grok"):
            prompt = f"Analyze these recent runs for {selected_vehicle} and predict ET at {target_da} ft DA.\n\n"
            for i in selected:
                r = recent[i]
                prompt += f"- {r['date']}: ET {r['et']:.3f}s @ {r.get('density_altitude','N/A')} ft DA"
                if r.get("notes"): prompt += f" | {r['notes']}"
                prompt += "\n"
            prompt += f"\nTarget Density Altitude: {target_da} ft\n"
            if context: prompt += f"Context: {', '.join(context)}\n"
            if extra: prompt += f"Notes: {extra}\n"
            prompt += "\nGive a smart prediction with reasoning and recommended safe dial-in."
            st.code(prompt, language="text")
            st.caption("Copy and send to me. Paste my reply below to save it.")
...
st.divider()
st.caption("Smart Slip v1.4 • Formula + Grok Hybrid Intelligence • Built for bracket racers")