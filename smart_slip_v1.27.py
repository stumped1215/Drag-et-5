...
def parse_import_block(block):
    """More robust parser that handles single-line output and extra text"""
    data = {}
    
    # First try splitting by newlines
    lines = block.strip().split("\n")
    
    # If we only got one line, try to extract key=value pairs using regex
    if len(lines) <= 1:
        # Find all key=value patterns anywhere in the text
...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.27 • Robust Import + Forced Weather")
...
# IMPORT FROM GROK (with robust parser + forced weather prompt)
with tab_import:
    ...
    # Much stricter prompt focused on weather + clean output
    ...
    if not st.session_state.import_success:
        ...
        if st.button("Import Run", type="primary", use_container_width=True):
            if not block or not block.strip():
                ...
                if "et" not in data:
                    st.error("Could not find ET in the block. Make sure Grok output key=value lines.")
                else:
                    conditions = {
...
st.divider()
st.caption("Smart Slip v1.27 • Built for bracket racers")