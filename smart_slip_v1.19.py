...
with tab_import:
    ...
    st.markdown("**Steps:** 1. Take photo → 2. Send to Grok → 3. Paste block here")

    prompt = f"""Car number: {car_number or '____'}. Extract data from the correct side.
...
with tab_history:
    ...
    if not runs:
        ...
        if not st.session_state.get("show_all_history", False):
...
st.divider()
st.caption("Smart Slip v1.19 • Built for bracket racers")